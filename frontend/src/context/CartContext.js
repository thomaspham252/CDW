...
export const CartProvider = ({ children }) => {
      ...
      const migratedCart = parsedCart
        .map((item) => {
          if (!item.variantId && item.id) {
            console.log(
              "[CartContext] Migrating item - adding variantId:",
              item,
            );
          ...
          if (!item.variantId && !item.id) {
            console.error(
              "[CartContext] Item missing both variantId and id:",
              item,
            );
  ...
  /** Xóa nhiều sản phẩm theo danh sách key */
  const removeManyFromCart = (keys) => {
    const keySet = new Set(Array.isArray(keys) ? keys : []);
    if (keySet.size === 0) return;
    setCart((prev) => prev.filter((i) => !keySet.has(i.key)));
  };
