...
const CheckoutPage = () => {
  const { clearCart, removeManyFromCart } = useCart();
  ...
  const checkoutItems = useMemo(() => selectedItems, [selectedItems]);
  const checkoutKeys = useMemo(
    () => checkoutItems.map((item) => item.key),
    [checkoutItems],
  );
  ...
  useEffect(() => {
    const loadAvailableCoupons = async () => {
      try {
        ...
        if (
          couponCode &&
          !coupons.some((coupon) => coupon.code === couponCode)
        ) {
          setCouponCode("");
          setCouponDiscount(0);
        }
  ...
  }, [selectedDistrictCode, selectedWardCode, subtotal, checkoutItems]);
  ...
  const handleClearAndFixCart = () => {
    if (
      window.confirm(
        "Xóa toàn bộ giỏ hàng và quay lại trang sản phẩm để thêm lại?",
      )
    ) {
      clearCart();
      navigate("/products");
  ...
  const handleSubmit = async (e) => {
    ...
    if (invalidItems.length > 0) {
      ...
      console.error("Full cart:", checkoutItems);
      setError(
        "Có sản phẩm trong giỏ hàng không hợp lệ. Vui lòng xóa giỏ hàng và thêm sản phẩm lại từ đầu.",
      );
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      removeManyFromCart(checkoutKeys);
      ...
      <div className="checkout-success-page">
        <div className="success-card">
          ...
          <p className="success-subtitle">
            Cảm ơn bạn đã mua sắm tại CDW Handmade.
          </p>
          <p className="order-code">
            Mã đơn hàng của bạn: <strong>#CDW-{orderSuccess.id}</strong>
          </p>
          ...
          <div className="order-details-summary">
            ...
            <div className="details-grid">
              ...
              <div className="details-val text-primary font-bold">
          ...
          {orderSuccess.paymentMethod === "BANK_TRANSFER" && (
            <div className="bank-transfer-instructions">
              ...
              <p className="inst-desc">
                Mở ứng dụng ngân hàng, quét mã VietQR bên dưới và kiểm tra đúng
                số tiền trước khi chuyển khoản.
              </p>
              ...
              <div className="bank-info-box">
                ...
                <div className="bank-info-row">
                  ...
                  <strong className="copyable">
                    {getTransferContent(orderSuccess.id)}
                  </strong>
                </div>
              </div>
              ...
              <div className="qr-code-wrapper">
                ...
                <span className="qr-hint">
                  <FontAwesomeIcon icon={icons.shield} /> Đơn hàng sẽ được xử lý
                  sau khi shop xác nhận thanh toán
                </span>
              </div>
            </div>
          )}
  ...
  return (
    <div className="checkout-page">
      <div className="checkout-container">
        ...
        {addressError && (
          <div className="checkout-error-banner">
            <FontAwesomeIcon icon={icons.warning} /> {addressError}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="checkout-form-layout">
          ...
          <div className="order-summary-section">
              ...
              {checkoutItems.map((item) => (
                <div key={item.key} className="order-item-row">
                  <div className="order-item-img">
                    <img
                      src={
                        item.image || "https://placehold.co/50x50?text=No+Image"
                      }
                      alt={item.name}
                    />
            ...
            <div className="coupon-section">
              ...
              {couponDiscount > 0 && (
                <div className="coupon-success">
                  <FontAwesomeIcon icon={icons.checkCircle} /> Mã giảm giá đã
                  được áp dụng!
                </div>
              )}
            </div>
            ...
            >
              {loading ? <span>Đang xử lý...</span> : <span>Đặt hàng</span>}
            </button>

            <p className="checkout-footer-hint">
              <FontAwesomeIcon icon={icons.shield} /> Thông tin cá nhân của bạn
